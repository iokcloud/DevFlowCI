...
## 必须审查的方面
1. **完整性**：代码是否被截断？检查以下几点：
   - 每个函数的函数体是否完整（有实际逻辑，不是空的或只有 pass）
   - 代码是否在不该结束的地方突然终止（如以逗号、冒号、未闭合表达式结尾）
   - 测试代码是否覆盖了所有公共函数
   - 如果发现截断，issues 中必须明确指出"代码被截断"或"函数不完整"
2. **功能**：代码是否实现了规格要求？
3. **质量**：类型注解、docstring、代码风格是否符合规范？
4. **安全**：是否存在 SQL 注入、硬编码密钥、路径遍历、未校验输入？
5. **异常处理**：外部调用是否有 try-except？异常是否正确传播？
6. **测试**：测试是否覆盖了核心路径和异常路径？
