...
@pytest.mark.asyncio
async def test_execute_alignment_insufficient_info_business_fallback(
    executor,
    base_state,
    seeded_project,
    monkeypatch,
):
    """AlignmentAgent 返回 insufficient_info 且有文档时，应兜底走商业计划。"""
    biz_context = {
        "document_type": "business",
        "analyzed_files": [{"file": "r.md", "summary": "市场规模 竞争分析"}],
        "overall_summary": "调研摘要",
        "source_files": [],
    }
    monkeypatch.setattr(
        "workflow.executor.analyze_project_context_structured",
    ...
    executor._alignment_agent.analyze_alternatives = AsyncMock(
        return_value=(
            {"status": "insufficient_info", "message": "文档信息不足"},
            {},
            [],
        )
    )
    mock_plan = {
        "executive_summary": "兜底商业摘要",
        "roadmap": [],
        "recommendations": "先做 MVP",
    }
    executor._business_planner.plan = AsyncMock(return_value=mock_plan)

    base_state["directory"] = "/fake/docs"
    result = await executor.execute_alignment(base_state)
...
@pytest.mark.asyncio
async def test_plan_only_keeps_alternative_plan(
    executor,
    base_state,
    seeded_project,
    sample_plan_module,
):
    """多方案规划时 alternative_plan 应写入 state。"""
    alt_module = {
        "module_name": "alt_api",
        "description": "备选 API 方案",
        "dependencies": [],
        "type": "backend",
    }
    executor._planner.plan_alternatives = AsyncMock(
        return_value=(
            {"modules": [sample_plan_module], "global_requirements": []},
            {"modules": [alt_module], "global_requirements": []},
            {"summary": "方案A更轻量"},
            [],
        )
    )
    ...
    assert result["status"] == "plan_ready"
    assert result.get("alternative_plan") is not None
    assert result["alternative_plan"]["modules"][0]["module_name"] == "alt_api"
